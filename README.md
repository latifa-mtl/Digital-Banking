...
**Login stuck on 401**
The interceptor was sending the token on the login request itself. Fix: whitelist `/auth/login` in `auth.interceptor.ts`.

**Transfer dropdown empty or shows "Unknown"**
`customerDTO` can be `null` in the backend's polymorphic JSON. The service normalises this — if still broken check `@CrossOrigin("*")` is on `BankAccountRestAPI` and the backend is running on port 8080.

**New account creation returns 400**
`/accounts/saveCurrent` and `/accounts/saveSaving` use `@RequestParam` not `@RequestBody`. Angular sends values as URL query parameters with a `null` body.

**Modals show backdrop blur but no content**
Modals placed inside a parent with `overflow: hidden` or a CSS `transform` get clipped regardless of `z-index`. All modals must sit **outside** the `.page` container in the template and use `position: fixed; inset: 0; z-index: 9999`.

**CORS errors**
Add to each controller or configure globally:
```java
@Bean
...
**Login button needs two clicks**
The `isLoggedIn()` redirect was running in the constructor before the router was ready. Moved to `ngOnInit()`.
